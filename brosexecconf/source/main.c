...
void drawAnimFrame(int frame_num){
	...
	if ( !(ctx = PNGU_SelectImageFromDevice(framepath)) ){
		myprintf("No se pudo abrir %s\n", framepath);
		return;
	}
...
#ifdef USEBACKGROUND
		/* Avanza la animacion cada ANIM_FRAME_SKIP vsyncs */
		anim_vsync_count++;
		if (anim_vsync_count >= ANIM_FRAME_SKIP){
			anim_vsync_count = 0;
			anim_curr_frame++;
			if (anim_curr_frame > ANIM_FRAME_COUNT) anim_curr_frame = 1;
			drawAnimFrame(anim_curr_frame);
		}
#endif
	}
